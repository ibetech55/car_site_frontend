...
import { IGetCityList, IGetStateList, IZipCode } from "../../Data/LocationDtos/LocationDtos";
...
export const getZipCode = createAsyncThunk(
  "location/getZipCode",
  async (zipCode: string, { rejectWithValue }): Promise<IZipCode> => {
    try {
      const { data } = await axios.get(`http://localhost:5010/location_api/zip_code/${zipCode}`);
      return data;
    } catch (error: any) {
      throw rejectWithValue(
        error.response.data.message
